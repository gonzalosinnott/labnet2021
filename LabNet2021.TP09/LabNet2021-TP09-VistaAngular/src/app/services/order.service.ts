...
import { Shippers } from '../models/order';
...
export class OrderService {
  ...
  updateOrder(order: Order): Observable<Order> {
    ...
    return this.http.put<Order>(this.url + '/UpdateOrder/', order, httpOptions);
  }
  ...
  deleteOrderById(orderId: number): Observable<number> {
    ...
    return this.http.delete<number>(
      this.url + '/DeleteOrder?id=' + orderId,
      httpOptions
    );
  }
  ...
  deleteData(user: Order[]): Observable<string> {
    const httpOptions = {
